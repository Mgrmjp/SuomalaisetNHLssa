...
from leagues import DELAdapter, SwissNLAdapter, CzechExtraligaAdapter, ICEHLAdapter
...
class OfficialLeagueCollector:
    ...
    def _build_hockeytech_headshot_url(
        self, client_code: str, player_id: str, size: str = "240x240"
    ) -> Optional[str]:
    ...
    def _build_ahl_profile_url(
        self, player_id: str, season_id: str, player_name: str
    ) -> Optional[str]:
    ...
    def _looks_like_player_photo(
        self, url: Optional[str], alt: Optional[str], player_name: str = ""
    ) -> bool:
        ...
        if any(
            ...
            for token in ("flag", "logo", "icon", "sprite", "blank", "placeholder")
        ):
            return False
        ...
        return any(
            token in normalized_url
            for token in ("player", "photo", "headshot", "portrait")
        )

    def _fetch_profile_photo(
        self, profile_url: Optional[str], player_name: str
    ) -> Optional[str]:
    ...
    def _get_chl_team_site_base(
        self, league_code: str, team_name: str
    ) -> Optional[str]:
        league_sites = CHL_TEAM_SITE_BASES.get(league_code, {})
        return league_sites.get(team_name)

    def _fetch_chl_roster_entries(
        self, league_code: str, team_name: str, team_id: str, season_id: str
        ...
        for match in pattern.finditer(response.text):
            ...
            entries[player_id] = {
                "headshot_url": match.group("headshot_url")
                .replace("https:\\/\\/", "https://")
                .replace("\\/", "/"),
                "profile_url": match.group("profile_url")
                .replace("https:\\/\\/", "https://")
                .replace("\\/", "/"),
            }
    ...
    def is_finnish(
        self, name: str, nationality: str = "", birthplace: str = ""
    ) -> bool:
    ...
    def _get_hockeytech_params(
        self, league_name: str, base_stats_url: str
    ) -> Dict[str, str]:
        ...
        try:
            ...
            if key_match and season_match:
                ...
                client_code = (
                    client_match.group(1) if client_match else league_name.lower()
                )
                print(f"  [{league_name}] Found key: {key}, season: {season}")
                return {"key": key, "season": season, "client_code": client_code}
        except Exception as e:
    ...
    def get_ahl_players(self, season: str = "2025-2026") -> List[Dict]:
        ...
        ht_params = self._get_hockeytech_params(
            "AHL", "https://theahl.com/stats/player-stats"
        )
        ...
        key = (
            ht_params.get("key", "ccb91f29d6744675")
            if ht_params
            else "ccb91f29d6744675"
        )
                    ...
                    players.append(
                        {
                            ...
                            "headshot_url": self._build_hockeytech_headshot_url(
                                "ahl", row.get("player_id", "")
                            ),
                            "profile_url": self._build_ahl_profile_url(
                                row.get("player_id", ""), ahl_season, name
                            ),
                            "source": "hockeytech",
                            "source_league": "ahl",
    ...
    def _parse_modulekit_skaters(
        self, data: dict, league: str, league_code: str, season_id: str
    ) -> List[Dict]:
        ...
        for p in skaters:
            ...
            profile_url = self._normalize_url(
                (roster_entry or {}).get("profile_url") or p.get("player_page_link")
            )
            headshot_url = self._normalize_url(
                (roster_entry or {}).get("headshot_url")
                or p.get("headshot_url")
                or p.get("headshot")
                or p.get("photo")
                or p.get("image")
            )
    ...
    def get_ohl_players(self, season: str = "2025-2026") -> List[Dict]:
        ...
        try:
            response = self.session.get(url, params=params, timeout=30)
            if response.status_code == 200:
                players = self._parse_modulekit_skaters(
                    response.json(), "OHL", "ohl", "83"
                )
            else:
            ...
            if response.status_code == 200:
                ...
                for row in stats:
                    ...
                    if not self.is_finnish(name, nat):
                        continue
                    team = (
                        info.get("team", {}).get("name", "Unknown")
                        if isinstance(info.get("team"), dict)
                        else "Unknown"
                    )
                    players.append(
                        {
                            ...
                            "headshot_url": self._extract_shl_headshot_url(
                                info.get("playerMedia")
                            ),
                            "source": "shl-statsv2",
                            "source_league": "shl",
                            "scraped_at": datetime.now().isoformat(),
    ...
    def get_liiga_players(self, season: str = "2025-2026") -> List[Dict]:
        """Get Liiga players from liiga.fi v2 API (including goalie stats)."""
        players = []

        year_end = (
            int(season.split("-")[1]) if "-" in season else datetime.now().year + 1
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for player in data if isinstance(data, list) else []:
                    ...
                    is_goalie = (
                        player.get("goalkeeper") == True or player.get("role") == "MV"
                    )

                    # v2 API returns savePercentage as percentage (89.4), convert to decimal (0.894)
                    raw_save_pct = player.get("savePercentage")
                    save_pct = None
                    if raw_save_pct is not None:
                        save_pct = (
                            raw_save_pct / 100 if raw_save_pct > 1 else raw_save_pct
                        )
                    ...
                    player_data = {
                        ...
                        "league": "Liiga",
                        "position": "G" if is_goalie else player.get("role", "F"),
                        "games_played": player.get("games", 0),
                        "goals": player.get("goals", 0) if not is_goalie else 0,
                        "assists": player.get("assists", 0),
                        "points": player.get("points", 0),
                        "plus_minus": player.get("plusMinus", 0),
                        "penalty_minutes": player.get("penaltyMinutes", 0),
                    ...
                    if is_goalie:
                        player_data["save_percentage"] = save_pct
                        player_data["goals_against_average"] = player.get(
                            "goalsAgainstAvg"
                        )
                        player_data["shutouts"] = player.get("shutOut", 0)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for player in data if isinstance(data, list) else []:
                    ...
                    name = player.get(
                        "name",
                        f"{player.get('firstname', '')} {player.get('surname', '')}",
                    ).strip()
                    ...
                    def sum_stat(key):
                        ...
                        if isinstance(v, dict):
                            return int(
                                (v.get("home", 0) or 0) + (v.get("away", 0) or 0)
                            )
                        return int(v or 0)
    ...
    def collect_all(self, season: str = "2025-2026") -> Dict:
        ...
        results["leagues"]["ahl"] = len(
            [p for p in results["players"] if p["source_league"] == "ahl"]
        )

        results["players"].extend(self.get_echl_players(season))
        results["leagues"]["echl"] = len(
            [p for p in results["players"] if p["source_league"] == "echl"]
        )

        results["players"].extend(self.get_ushl_players(season))
        results["leagues"]["ushl"] = len(
            [p for p in results["players"] if p["source_league"] == "ushl"]
        )

        results["players"].extend(self.get_nahl_players(season))
        results["leagues"]["nahl"] = len(
            [p for p in results["players"] if p["source_league"] == "nahl"]
        )

        # Canada
        print("\n--- CANADIAN LEAGUES ---")
        results["players"].extend(self.get_ohl_players(season))
        results["leagues"]["ohl"] = len(
            [p for p in results["players"] if p["source_league"] == "ohl"]
        )

        results["players"].extend(self.get_whl_players(season))
        results["leagues"]["whl"] = len(
            [p for p in results["players"] if p["source_league"] == "whl"]
        )

        results["players"].extend(self.get_qmjhl_players(season))
        results["leagues"]["qmjhl"] = len(
            [p for p in results["players"] if p["source_league"] == "qmjhl"]
        )

        # Europe
        print("\n--- EUROPEAN LEAGUES ---")
        results["players"].extend(self.get_khl_players(season))
        results["leagues"]["khl"] = len(
            [p for p in results["players"] if p["source_league"] == "khl"]
        )

        results["players"].extend(self.get_shl_players(season))
        results["leagues"]["shl"] = len(
            [p for p in results["players"] if p["source_league"] == "shl"]
        )

        results["players"].extend(self.get_liiga_players(season))
        results["leagues"]["liiga"] = len(
            [p for p in results["players"] if p["source_league"] == "liiga"]
        )

        results["players"].extend(self.get_del_players(season))
        results["leagues"]["del"] = len(
            [p for p in results["players"] if p["source_league"] == "del"]
        )

        results["players"].extend(self.get_nl_players(season))
        results["leagues"]["nl"] = len(
            [p for p in results["players"] if p["source_league"] == "nl"]
        )

        results["players"].extend(self.get_czech_players(season))
        results["leagues"]["czech"] = len(
            [p for p in results["players"] if p["source_league"] == "czech"]
        )

        results["players"].extend(self.get_icehl_players(season))
        results["leagues"]["icehl"] = len(
            [p for p in results["players"] if p["source_league"] == "icehl"]
        )
...
def main():
    ...
    if results["players"]:
        ...
        for i, p in enumerate(results["players"][:20], 1):
            print(
                f"  {i}. {p['name']} ({p['league']}, {p['team']}): {p['goals']}G + {p['assists']}A = {p['points']}P"
            )
