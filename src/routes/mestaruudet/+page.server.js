...
export async function load({ fetch }) {
    ...
    async function fetchWinnerData(id) {
        try {
            ...
            const rawName =
                `${data.firstName?.default || ''} ${data.lastName?.default || ''}`.trim()
            const name = correctFullName(rawName)

            // 2. Find Stanley Cup Award
            const cupAward = data.awards?.find(
                (/** @type {{ trophy: { default: string; }; }} */ a) =>
                    a.trophy?.default === 'Stanley Cup'
            )
            ...
            const years = cupAward.seasons.map(
                (/** @type {{ seasonId: number; gameTypeId: number; }} */ seasonInfo) => {
                    ...
                    if (playOffStats?.teamName?.default) {
                        teamName = playOffStats.teamName.default
                    } else if (playOffStats?.teamCommonName?.default) {
