...
function getTeamColorHueScore(hue) {
    ...
    if (
        (normalizedHue >= 0 && normalizedHue <= 30) ||
        (normalizedHue >= 330 && normalizedHue <= 360)
    )
        return 90
...
function generateAccentColor(primaryColor, secondaryColor) {
    try {
        ...
        const finalR =
            accentR > 200 ? Math.max(accentR - 30, primary.r + 20, secondary.r + 20) : accentR
        const finalG =
            accentG > 200 ? Math.max(accentG - 30, primary.g + 20, secondary.g + 20) : accentG
        const finalB =
